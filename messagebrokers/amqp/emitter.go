...
func NewAMQPEventEmitterFromEnvironment() (messagebrokers.EventEmitter, error) {
	var url string
	var exchange string
...
func NewAMQPEventEmitter(conn *amqp.Connection, exchange string) (messagebrokers.EventEmitter, error) {
...
func (a *amqpEventEmitter) Emit(event messagebrokers.Event) error {
	channel, err := a.connection.Channel()
	if err != nil {
		return err
	}
