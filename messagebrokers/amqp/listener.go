...
type amqpEventListener struct {
	...
	exchange   string
	queue      string
	mapper     messagebrokers.EventMapper
}
...
func NewAMQPEventListenerFromEnvironment() (messagebrokers.EventListener, error) {
	var url string
	var exchange string
	var queue string
...
func NewAMQPEventListener(conn *amqp.Connection, exchange string, queue string) (messagebrokers.EventListener, error) {
	listener := amqpEventListener{
		connection: conn,
		exchange:   exchange,
		queue:      queue,
		mapper:     messagebrokers.NewEventMapper(),
	}
...
func (l *amqpEventListener) Listen(eventNames ...string) (<-chan messagebrokers.Event, <-chan error, error) {
	...
	events := make(chan messagebrokers.Event)
...
func (l *amqpEventListener) Mapper() messagebrokers.EventMapper {
	return l.mapper
}
